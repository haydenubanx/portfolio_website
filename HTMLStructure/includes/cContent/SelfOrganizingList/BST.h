...
private:
    ...
    BSTNode<Key, E> *removehelp2(BSTNode<Key, E> *, const Key &);
...
//    BSTNode<Key, E>* findInOrderSuccessor(BSTNode<Key, E>* root, BSTNode<Key, E>* node);
    ...
    void setThreaded(BSTNode<Key, E> *currentNode) {
        ...
        if (currentNode != NULL) {
            ...
            if (currentNode->left() == NULL) {
                ...
                if(!getParent(currentNode)->getLeftThreaded() && currentNode->key() <= getParent(currentNode)->key()) {

                    getParent(currentNode)->setLeftThreaded(true);
                }
                else if(!getParent(currentNode)->getRightThreaded() && currentNode->key() > getParent(currentNode)->key()) {

                    getParent(currentNode)->setRightThreaded(true);
                }
                ...
                if (getmin(leftPointerToSet)->key() == currentNode->key()) {

                    leftPointerToSet = NULL;
                    currentNode->setLeftThreaded(false);
                }

                else {
                    //Otherwise if the current node is less than the temp variable
                    while (currentNode->key() <= leftPointerToSet->key() && leftPointerToSet->key() != root->key()) {
...
//                    cout << "The left pointer at key " << currentNode->key() << " was set to point at " << leftPointerToSet->key() << endl;
            ...
            if (currentNode->right() == NULL) {
                ...
                if (getmax(rightPointerToSet)->key() == currentNode->key()) {
                    rightPointerToSet = NULL;
                    currentNode->setRightThreaded(false);
                }
...
BSTNode<Key, E> *BST<Key, E>::inserthelp(
        BSTNode<Key, E> *node, const Key &k, const E &val) {
    if (node == nullptr) {
        return new BSTNode<Key, E>(k, val, NULL, NULL);
    } else if (k <= node->key()) {
        node->setLeft(inserthelp(node->left(), k, val));
    } else if (k > node->key()) {
        node->setRight(inserthelp(node->right(), k, val));
    }
...
template<typename Key, typename E>
BSTNode<Key, E> *BST<Key, E>::
removehelp2(BSTNode<Key, E> *rt, const Key &k) {

    bool childWasLeftOfParent = false;
    bool childWasRightOfParent = false;

    if (rt == NULL) return NULL;
    else if (k < rt->key())
        rt->setLeft(removehelp2(rt->left(), k));
    else if (k > rt->key())
        rt->setRight(removehelp2(rt->right(), k));
    else {
        BSTNode<Key, E> *parent = getParent(rt);

        //if child is left of parent, remove left thread of parent from child
        if (rt->key() < parent->key()) {
            parent->clearLeftThread();

            //Update left thread boolean
            bool tempToPass = false;
            childWasLeftOfParent = true;
            parent->setLeftThreaded(tempToPass);
        }

            //if child is right of parent, remove right thread of parent from child
        else if (rt->key() > parent->key()) {
            parent->clearRightThread();

            //Update left thread boolean
            bool tempToPass = false;
            childWasRightOfParent = true;
            parent->setRightThreaded(tempToPass);
        }

        //if not a leaf, assign parent thread to child node
        if (!rt->isLeaf()) {

            //if left threaded only
            if (rt->getLeftThreaded() && !rt->getRightThreaded()) {
                if (childWasLeftOfParent) {
                    parent->setLeft(rt->left());
                } else if (childWasRightOfParent) {
                    parent->setRight(rt->left());
                }
            }
                //if right threaded only
            else if (rt->getRightThreaded() && !rt->getLeftThreaded()) {
                if (childWasLeftOfParent) {
                ...
                //get minimum of right subtree and set this to new child node of parent
                if (childWasLeftOfParent) {
                    parent->setLeft(getmin(rt->right()));
                } else if (childWasRightOfParent) {
                    parent->setRight(getmin(rt->right()));
                }

            }

        }
            //If leaf node, just set to null
        else {
            rt = NULL;
        }
    }
